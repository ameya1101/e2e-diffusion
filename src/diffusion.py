...
class PointDiffusion(nn.Module):
    def __init__(self, num_deposits=300, model_config=None) -> None:
        ...
        self.timesteps = (
            torch.arange(start=0, end=self.num_steps + 1, dtype=torch.float32)
            / self.num_steps
            + 8e-3
        )
...
class DDIMSampler(nn.Module):
    def __init__(
        ...
        self.init_x = torch.randn(self.data_shape)
    ...
    def sample(self):
        ...
        for time_step in torch.arange(start=self.num_steps - 1, end=0, step=-1):
            ...
            t_embedding = self.diffusion.time_embedder(batched_time_step)
            self.model.eval()
            ...
            x = mean + torch.exp(0.2 * log_var) * z
