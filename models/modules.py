...
class Down(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, emb_dim: int = 128) -> None:
...
class UNet(nn.Module):
    def __init__(
        self, c_in: int = 3, c_out: int = 3, time_dim: int = 128, device: str = "cuda"
    ) -> None:
    ...
    def pos_encoding(self, t, channels):
        inv_freq = 1.0 / (
            10_000
            ** (torch.arange(0.0, channels, 2, device=self.device).float() / channels)
        )
...
if __name__ == '__main__':
    ...
    t = x.new_tensor([300] * x.shape[0])
    print(model(x, t).shape == x.shape)